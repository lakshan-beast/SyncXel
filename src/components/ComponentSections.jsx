...
export default function AllComponents() {
  ...
  // 🎯 Search + Category දෙකෙන්ම Filter වෙන Logic එක
  ...
  return (
    <section className="py-2 px-4 sm:px-8 max-w-7xl mx-auto space-y-8 relative">
      {/* Background Accent Glow */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-125 h-75 bg-syncxel-cyan/5 blur-[150px] pointer-events-none rounded-full" />
      ...
      {/* 2. SEARCH & CONTROL BAR (නව එකතු කිරීම) */}
      <div className="space-y-4 relative z-10 max-w-4xl mx-auto">
        {/* Search Input Box */}
        <div className="relative max-w-md mx-auto">
          <HiOutlineMagnifyingGlass className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-syncxel-textMuted" />
          <input
            type="text"
            placeholder="Search components (e.g. Button, Hero, Input)..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-11 pr-10 py-3 rounded-xl bg-syncxel-card/80 border border-syncxel-border/80 text-white placeholder-syncxel-textMuted/60 text-sm focus:outline-none focus:border-syncxel-cyan transition-all shadow-lg"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery("")}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-syncxel-textMuted hover:text-white p-1">
      ...
      {filteredItems.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {/* 🎯 Fix: මෙතන componentsData වෙනුවට filteredItems දාලා තියෙන්නේ */}
        ...
        /* 4. NO RESULTS FOUND AREA */
        <div className="text-center py-16 bg-syncxel-card/30 rounded-2xl border border-syncxel-border/40 space-y-3">
          <p className="text-lg text-white font-semibold">
            No components found for "{searchQuery}"
          </p>
          <p className="text-xs text-syncxel-textMuted">
            Try searching for something else or reset your active category
            filters.
          </p>
          <button
            onClick={() => {
            ...
            className="px-4 py-2 rounded-xl bg-syncxel-cyan/10 border border-syncxel-cyan/30 text-syncxel-cyan text-xs font-semibold hover:bg-syncxel-cyan hover:text-black transition-all">
            Reset All Filters
          </button>
        </div>
      )}
